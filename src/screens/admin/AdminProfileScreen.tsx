...
export const AdminProfileScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const display: any = user || {};
  const avatarUrl = display?.avatarUrl || display?.avatar || '';
  const isSuperAdmin = display?.level === 'SUPER_ADMIN';
  const roleLabel = isSuperAdmin ? 'SUPER ADMIN' : 'ADMIN';
  const joinedDate = display?.createdAt ? new Date(display.createdAt).toLocaleDateString('en-IN') : '—';
  const emailVerified = !!(display?.emailVerified ?? display?.isEmailVerified);

  return (
    <ScrollView
      style={styles.container}
      contentContainerStyle={{ paddingBottom: 110 }}
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refreshProfile()} colors={[COLORS.primary]} />}
    >
      <StatusBar barStyle="light-content" />

      <LinearGradient colors={[COLORS.midnight, COLORS.primaryDark, COLORS.primary]} style={styles.headerGradient}>
        <TouchableOpacity
        ...
        <Text style={styles.profileName}>{display?.name || 'Administrator'}</Text>
      ...
      <View style={styles.sectionHeaderRow}>
        ...
        <TouchableOpacity style={styles.sectionEditBtn} onPress={() => navigation.navigate('EditAdminProfile')}>
          <Ionicons name="create-outline" size={16} color={COLORS.primary} />
        </TouchableOpacity>
      </View>
      <View style={styles.card}>
        <EmailRow email={display?.email || '—'} verified={emailVerified} styles={styles} COLORS={COLORS} />
        <InfoRow icon="call-outline" label="Phone" value={display?.phone || '—'} styles={styles} COLORS={COLORS} />
        <InfoRow icon="ribbon-outline" label="Admin Level" value={roleLabel} styles={styles} COLORS={COLORS} />
      ...
      <View style={styles.card}>
        ...
        <MenuItem icon="key-outline" label="Security & Account" onPress={() => navigation.navigate('ChangePassword')} styles={styles} COLORS={COLORS} />
